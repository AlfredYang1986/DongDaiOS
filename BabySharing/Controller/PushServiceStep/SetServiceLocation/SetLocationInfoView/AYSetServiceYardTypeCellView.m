//
//  AYSetServiceYardTypeCellView.m
//  BabySharing
//
...
#import "AYSetServiceYardTypeCellView.h"
...
@implementation AYSetServiceYardTypeCellView {
	UILabel *titleLabel;
	UILabel *subLabel;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
	...
	if (self) {
		...
		subLabel = [Tools creatUILabelWithText:nil andTextColor:[Tools garyColor] andFontSize:313 andBackgroundColor:nil andTextAlignment:NSTextAlignmentLeft];
...
- (void)setUpReuseCell {
	id<AYViewBase> cell = VIEW(@"SetServiceYardTypeCell", @"SetServiceYardTypeCell");
...
- (id)setCellInfo:(id)args {
	...
	NSString *desc = [args objectForKey:kAYServiceArgsYardType];
	if (desc.length == 0) {
		desc = @"请选择";
