//
//  AYSearchFilterCellDefines.h
//  BabySharing
//
//  Created by BM on 9/1/16.
//  Copyright © 2016 Alfred Yang. All rights reserved.
//

#ifndef AYSearchFilterCellDefines_h
#define AYSearchFilterCellDefines_h

static NSString* const kAYSearchFilterCellName                      = @"SearchFilterCell";

static NSString* const kAYSearchFilterCellCellKey                   = @"kAYSearchFilterCellCellKey";
static NSString* const kAYSearchFilterCellTitleKey                  = @"kAYSearchFilterCellTitleKey";

#endif /* AYSearchFilterCellDefines_h */
